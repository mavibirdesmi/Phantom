...
def flash_attention(
    ...
    if q_lens is None:
        ...
        torch._check_is_size(q.size(0) * q.size(1))
        ...
        q_lens_t = torch.tensor(q_lens, dtype=torch.int32, device=q.device)
    ...
    if k_lens is None:
        ...
        k_lens_t = torch.tensor(k_lens, dtype=torch.int32, device=k.device)
